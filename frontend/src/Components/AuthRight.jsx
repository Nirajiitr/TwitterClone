    ...
    <div className='xl:w-[49%]'>
        ...
        <p className='md:text-[70px] font-mono font-bold md:mt-20 vsm:text-[40px]  text-slate-200'>Happening now</p>
          <div className='w-80 '>
            ...
            <button className='h-10 text-white bg-blue-500 rounded-3xl flex justify-center items-center gap-1 w-80 my-2 ' onClick={()=>setSignUpModal(true)} >Create account</button>
            ...
            <button className='h-10 text-blue-500 bg-black border-[1px] border-slate-700 rounded-3xl flex justify-center items-center gap-1 w-80 my-2 font-bold font-mono hover:bg-slate-900 'onClick={()=>setLoginModal(true)} >Sign in</button>
          </div>
        
          
            <AuthModal showSignUpModal = {signUpModal} showLoginModal={loginModal} onClose ={()=>(setSignUpModal(false)||setLoginModal(false)) }  /> 
